...
Current Features:
  ...
  Options for changing list sizes, number of lists, and others. See docstring
    of compSortTest for more info.
  Automatic error detection, and notification along with automatic debug list
    (10 item list) testing for easy debugging.
Future Features:
  Options for pathological data (sorted lists, almost sorted lists, lists with
    high repetition, near reversed lists, etc.)
  Better debugging with pathological data options and more helpful errors.
...
def compSortTest(sortList, max_size_order=7, mult_list_size=True,
    ...
    Set max_size_order to the largest list size you want where the size is
       10 ** max_size_order. Lengths start at 10 ** 2, so the default 7
       generates 5 test lists.
    ...
    Set try_debug_list to False to not try the sort on a small list if it
        fails the assertion test.
    ...
    if mult_list_size:
        for i in range(2, max_size_order):
            ...
            print "Generating {0} random ints with max size {1}...".format(
    ...
    # Will eventually contain sort_name:list_of_times_in_order for lookup and
    # use on the leaderboard.
    times = {}
    # Will eventually contain sort_name:sorted_lists_in_order for lookup during
    # sort checking.
    sorted_lists = {}
    # We want to keep the initial list but want to be able to remove
    # bad sorts from the loop.
    ...
    for list_index, unsorted in enumerate(unsorted_lists):
        ...
        # Copy made here to avoid a nasty bug where the for loop skips on bad
        # sorts
        for sort in sorts_to_test[:]:
            sort_name = sort.__name__
            if verbose_timing:
                print "For {0} items, trying {1}...".format(len(unsorted),
                        sort.__name__)
            ...
            if check_sort:
                try:
                    ...
                    print ''
                    print "{0} did NOT work for the random list with {1} integers.".format(sort_name, len(unsorted))
                    if try_debug_list:
    ...
    # Bad sorts have been removed. Name change is for readability.
    ...
    for index, unsorted in enumerate(unsorted_lists):
        ...
        print "For list {0}, with length {1} and range {2}: ".format(index + 1,
